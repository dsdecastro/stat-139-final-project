...
Add description of all the variables considered in our analyses 
...
```{r, eval=T}
t.test(total.cases ~ religious, data=full.cases)
```
...
```{r, eval=T}
t.test(total.cases ~ catholic, data=full.cases)
```
...
## Reviewing the Assumptions of Linear Regression
