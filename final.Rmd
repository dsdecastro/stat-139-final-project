...
* **religious.affiliation** "Indicates religious affiliation (denomination) for private not-for-profit institutions that are religiously affiliated" [IPEDS]. 

* **religious** Indicator with "Yes" if the institution is religious, "No" if not. 

* **catholic** Indicator with "Yes" if the institution is Catholic, "No" if not. 
...
* **grad.rate** "Graduation rate of first-time, full-time degree or certificate-seeking students" [IPEDS].
...
In the above plot for the model for `total.cases`, the spread of the residuals appears much more consistent across the entire range of fitted values. If one looks carefully enough, however, they will see that the same behavior observed in the preceding plot is present in this one, the difference being that there are fewer schools that reported very low case counts (0, 1, or 2). This is likely due both to the fact that, with an extra five months of data, our response took on enough different values to begin to behave more like a continuous variable, and to the not insignificant number of schools that simply did not report case data for the spring semester (297 out of 1855 observations).
...
`religious` had a statistically significant interactive term with 5 other variables, which were: `percent.black`, `percent.american.native`, `percent.fin.aid`, `on.campus.housing`, and `mask.mandated.days`. For example, the term `religiousYes:mask.mandated.days` in the summary results of the interaction model had a coefficient of 2.275e-03 and a p-value of 6.43e-05. This implies that it is statistically significant in this model, and for religiously affiliated schools, a one-day increase in mask mandates correlates with an increase in log Covid cases compared to its non-religious counterpart (specifically $e^{0.002275}$. This result is interesting, and could prompt further investigation in a different paper. 
...
The results of the five step models explored come to very similar conclusions: though `religious` alone was sometimes included as a predictor and sometimes not, it was never statistically significant in the models. Conversely, five interactive terms stood out, specifically: `religiousYes:mask.mandated.days`, `religiousYes:on.campus.housingYes`, `religiousYes:tuition`, `religiousYes:log(percent.asian + epsilon)`, and `religiousYes:percent.two.more.races`. Three of these (`mask.mandated.days`, `on.campus.housing`, and `percent.asian`) were statistically significant in four of the models, and the remaining two were significant in three of those four. The fifth model, the forward interaction model starting from the intercept model, had no statistically significant interactive terms with `religious`. 

Of these interactions, `mask.mandated.days` was always the most statistically significant with `religious`. Next was `on.campus.housing`, then typically `percent.asian`. This makes sense given that they appeared the most as significant in the models. 

WHAT ARE THE SOCIAL REASONS FOR THIS? 
...
We thought that the backward step model may be the worst for our data due to the high collinearity, as the procedure would force the model to consider and keep collinear variables because it starts with them. Conversely, a forward stepwise model could entirely omit them from the procedure. While the run time for the backward step model was the greatest and is indicative of considering so many interactive terms, it has a higher AIC and does not seem to differ much with variable significance from other step models we considered. 
...
for (i in 1:length(ranef(er1)[[1]][,1])) {
  intercept = ranef(er1)[[1]][i,1]
  abline(fixef(er1)[1]+intercept, fixef(er1)[6], col = rgb(0,color_list[i], 1-color_list[i]/2, .6))
}
...
We now fit Poisson regression models using the same formulas as the most notable Gaussian models, and compare the lists of coefficient estimates related to religious affiliation marked as statistically significant by the two approaches. The tables below illustrates the results of these analyses. 

```{r, eval=T}
modelsummary(list("base gaussian"=lm6, "weighted base poisson"=p2),
             estimate="{estimate}", 
             statistic="{p.value} {stars}",
             shape=term ~ model + statistic, gof_omit="IC")

cov.p2 <- vcovHC(p2, type="HC0")
std.err <- sqrt(diag(cov.p2))
r.est <- cbind(Estimate=coef(p2), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(p2)/std.err), lower.tail=FALSE))
r.est
```

```{r, eval=T}
religiousInteraction.poisson <- glm(cases ~ religious * 
                                      (tuition + log(total.headcount + 
    epsilon) + log(percent.american.native + epsilon) + log(percent.asian + 
    epsilon) + log(percent.black + epsilon) + log(percent.hispanic.latino + 
    epsilon) + log(percent.pacific.islander + epsilon) + percent.white + 
    percent.two.more.races + percent.women + grad.rate + log(100 - 
    ...
    hs.equivalent.degree), data=full.cases, family="poisson", 
    weights=1/total.headcount)
# plot(religiousInteraction.poisson, which=1)
modelsummary(list("religiousInteraction gaussian"=religiousInteraction, 
                  "weighted religiousInteraction poisson"=religiousInteraction.poisson),
             estimate="{estimate}", 
             statistic="{p.value} {stars}",
             shape=term ~ model + statistic, gof_omit="IC", 
             coef_omit="^(?!religious)")
...
cov <- vcovHC(religiousInteraction.poisson, type="HC0")
std.err <- sqrt(diag(cov))
r.est <- cbind(Estimate=coef(religiousInteraction.poisson), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(religiousInteraction.poisson)/std.err), lower.tail=FALSE))

significant_pval = c()
significant_coeff = c()
for (i in 1:length(r.est[,1])) {
  if (!is.na(r.est[i,3]) && r.est[i,3] < 0.05) {
    significant_coeff <- c(significant_coeff, rownames(r.est)[i])
    significant_pval <- c(significant_pval, r.est[i,3])
  }
}
data.frame(coefficients = significant_coeff,
           p.value = significant_pval)
...
fullInteraction.poisson <- glm(cases ~ (religious + tuition + log(total.headcount + 
    ...
    percent.fin.aid + epsilon) + on.campus.housing + gap20repub + 
    private + percent.student.loan + mask.mandated.days + occupational.degree + 
    hs.equivalent.degree)^2, data=full.cases, family="poisson")

modelsummary(list("fullInteraction gaussian"=fullInteraction, 
                  "weighted fullInteraction poisson"=fullInteraction.poisson),
             estimate="{estimate}", 
             statistic="{p.value} {stars}",
             shape=term ~ model + statistic, gof_omit="IC", 
             coef_omit="^(?!religious)")
...
modelsummary(list("gaussian"=fI.lasso.remaining, 
                  "poisson"=fI.lr.poisson),
             estimate="{estimate}", 
             statistic="{p.value} {stars}",
             shape=term ~ model + statistic, gof_omit="IC",
             coef_omit="^(?!religious)")

cov.fI.poisson <- vcovHC(fI.lr.poisson, type="HC0")
std.err <- sqrt(diag(cov.fI.poisson))
r.est <- cbind(Estimate= coef(fI.lr.poisson), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(fI.lr.poisson)/std.err), lower.tail=FALSE),
LL = coef(fI.lr.poisson) - 1.96 * std.err,
UL = coef(fI.lr.poisson) + 1.96 * std.err)
```

```{r, eval=T}
normalized.df <- full.cases[,c("cases", "religious", "tuition", "total.headcount", "percent.american.native", "percent.asian", "percent.black", "percent.hispanic.latino", "percent.pacific.islander", "percent.white", "percent.two.more.races", "percent.women", "grad.rate", "percent.fin.aid", "on.campus.housing", "gap20repub", "private", "percent.student.loan", "mask.mandated.days", "occupational.degree", "hs.equivalent.degree", "state")]
normalized.df$religious <- normalized.df$religious == "Yes"
normalized.df$on.campus.housing <- normalized.df$on.campus.housing == "Yes"
normalized.df$private <- normalized.df$private == "Yes"
normalized.df$occupational.degree <- normalized.df$occupational.degree == "Yes"
normalized.df$hs.equivalent.degree <- normalized.df$hs.equivalent.degree == "Yes"

for (i in 1:length(colnames(normalized.df))) {
  if (!is.factor(normalized.df[,i]) && colnames(normalized.df)[i] != "cases") {
    normalized.df[,i] <- scale(normalized.df[,i])
  }
}

random.offset <- glmer(cases ~ 1 + religious + (1|state) + offset(log(total.headcount)), 
      data=normalized.df, family="poisson")
```

```{r, eval=T}
random.offset <- glmer(cases ~ 1 + religious + tuition + 
      log(percent.american.native + epsilon) + log(percent.asian + 
      epsilon) + log(percent.black + epsilon) + log(percent.hispanic.latino + 
      epsilon) + log(percent.pacific.islander + epsilon) + percent.white + 
      percent.two.more.races + percent.women + grad.rate + log(100 - 
...
- Brief summary of first three sections: Group Testing, Assumptions, 2020/21/total.cases decision
- Linear model recap, determining the significance of `religious` relies a lot on its interactions 
- AIC step comes to similar conclusions? 

Discussing inferences: 
- Which coefficients related to `religious` (linear and interaction effects) did we find to be significant at each stage of our analyses? 
- Where the findings differ, which models do we choose to give more credit to? Which findings are we more confident in (based on number of models that support the significance of a predictor)? 
- Giving the interpretation of every predictor that is discussed? 
...
plot(log(total.cases + epsilon) ~ tuition, data=full.cases,
     main="tuition")
plot(log(total.cases + epsilon) ~ log(total.headcount+epsilon), data=full.cases,
