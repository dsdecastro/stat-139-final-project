...
for (i in 1:length(r.est[,1])) {
  predictor_name = substr(rownames(r.est)[i], 1, 9)
  if (!is.na(r.est[i,3]) && r.est[i,3] < 0.05 && predictor_name == "religious"){
...
std.err <- sqrt(diag(cov))
r.est <- cbind(Estimate=coef(fullInteraction.poisson)[-36], "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(fullInteraction.poisson)[-36]/std.err), lower.tail=FALSE))
...
std.err <- sqrt(diag(cov))
r.est <- cbind(Estimate=coef(fI.lr.poisson)[-35], "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(fI.lr.poisson)[-35]/std.err), lower.tail=FALSE))
