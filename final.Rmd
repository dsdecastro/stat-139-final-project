...
=======
df = na.omit(full.cases[c("cases", "religious", "tuition", "total.headcount","percent.pacific.islander", "percent.hispanic.latino", "percent.white","percent.two.more.races", "percent.women", "grad.rate", "percent.fin.aid", "percent.american.native", "percent.asian","on.campus.housing", "gap20repub", "private", "percent.student.loan", "mask.mandated.days", "percent.black", "occupational.degree", "hs.equivalent.degree")])
>>>>>>> e8827d5bdb0393eca00aa1523a1b582e171c2451
...
r.est <- data.frame("Poisson Estimate"=unname(coef(fullInteraction.poisson)[-36]), 
                    Poisson.p.value= 2 * pnorm(abs(coef(fullInteraction.poisson)[-36]/std.err), lower.tail=FALSE))
r.est$coefficient <- rownames(r.est)
...
std.err <- sqrt(diag(cov))
r.est <- data.frame("Poisson Estimate"=unname(coef(fI.lr.poisson)[-35]), 
                    Poisson.p.value= 2 * pnorm(abs(coef(fI.lr.poisson)[-35]/std.err), lower.tail=FALSE))
r.est$coefficient <- rownames(r.est)
