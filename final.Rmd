...
- Backward step model: For this model, we started with `fullInteraction`, stepping backwards until a reduction in AIC was no longer possible. This process removed 116 predictors, leaving the resulting model, `back.step`, with 94 predictors. This list includes `religiousYes`, as well as interaction between `religious` and eight other predictors: `tuition`, `log(total.headcount + epsilon)`, `log(percent.american.native + epsilon)`, `log(percent.asian + epsilon)`, `log(percent.black + epsilon)`, `percent.two.more.races`, `on.campus.housingYes`, and `mask.mandated.days`. Of the nine predictors related to `religious` included in this model, only 4 were significant at the 0.05 level: `religiousYes`, `religiousYes:log(percent.asian + epsilon)`, `religiousYes:on.campus.housingYes`, and `religiousYes:mask.mandated.days`. Thus, according to this model, the statistically significant relationship between `religious` and `cases` changes in nature based on the predictors `log(percent.asian + epsilon)`, `on.campus.housingYes`, and `mask.mandated.days`. Also worth noting is that the backward-stepping method of sequential variable selection might not be the best for our purposes: As we wish to trim down our incredibly large list of predcitors, starting with the complete list of predictors — rather than starting with a smaller list, or even none at all — might lead the algorithm to converge to a local min that keeps many more predictors than a forward-stepping or bidirectionally stepping method. 
...
- Forward step 1: For this model, we stepped forward from an intercept model. As a result, `forward.step1` incorporated 67 predictors. Rather tellingly, this list of predictors did not include `religiousYes`, or any interaction term with `religious`. This could be taken as evidence that `religious` really is not a relevant predictor at all. 

- Bidirectional step: For this model, we used our base model as our starting point, and stepped in both directions (alternating forwards and backwards), until an improvement in AIC was no longer possible. As a result, 55 predictors were added to the base model to produce `both.step`, a model with 75 total predictors. This list of predictors for this model was extremely similar to the list of predictors included in the `forward.step` model; only four predictors were different between the two models, none of which were related to `religious`. Predictably, the same interaction terms with `religious` that were significant in `forward.step` were statistically significant in this model as well, and `religiousYes` was again not statistically significant. This model corroborates the idea that whether an institution is religious changes the relationship between `cases` and these other predictors, but that there is no relationship between the predictor `religious` and the response `cases`.  

As we compare the final AIC values of all four models below, we see that the`back.step` model has the lowest AIC; however, the `forward.step` and `both.step` models do have comparable values for AIC.
...
back.step.terms <- attributes(back.step$terms)$term.labels
forward.step.terms <- attributes(forward.step$terms)$term.labels
forward.step1.terms <- attributes(forward.step1$terms)$term.labels
forward.step2.terms <- attributes(forward.step2$terms)$term.labels
both.step.terms <- attributes(both.step$terms)$term.labels
```

\begin{center}
```{r, eval=T, render=lemon_print}
# Hard-coded for now to reduce knit-time 
data.frame(model=c("back.step", "forward.step", "forward.step1", 
                   "both.step"), 
           "AIC"=c(4816.395, 4833.366, 4910.775, 4828.786))
```
\end{center}
...
Formally test whether the effect of university religious affiliation (on 2020 Covid cases) depends on `gap20repub`: (it does)

```{r}

lmer3 = lmer(log(cases+1)~gap20repub*religious+(1|state),data=full.cases)
lmer3b = lmer(log(cases+1)~gap20repub+religious+(1|state),data=full.cases)
anova(lmer3b,lmer3)
...
```{r}
# I am trying to get a graph where we can plot each state's line of a random intercept model (should be straight lines I believe)
lmer2 = lmer(log(cases+1) ~ religious + log(total.headcount + 1) + (1|state),data=full.cases)

plot(full.cases$total.headcount, full.cases$cases)
abline(summary(lmer2))

state.df = full.cases[full.cases$state == "Alabama",]
df2 = data.frame(state.df$total.headcount, predict(lmer2, state.df))
...
SOME EXPERIMENTING WITH MIXED-EFFECTS MODELS 
```{r, eval=T}
lmer.test <- lmer(log(cases + epsilon) ~ religious + tuition + log(total.headcount + 
